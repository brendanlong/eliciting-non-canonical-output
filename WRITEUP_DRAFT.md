...
*Draft for the MATS application doc, to be rewritten in Brendan's voice. Figures come from `scripts/figures.py`, the examples from `scripts/random_examples.py`, the composition table from `scripts/flag_composition.py`. Numbers are from the generated tables in `RESULTS.md` except where noted: the divergence numbers (section 5) and RL-Zero steps 600 and 1400 come from `figures/data.json`, computed from the HF dataset, which is ahead of `RESULTS.md` for those cells. `[todo]` marks what needs Brendan.*
...
**Figure 1.** Rollouts with at least one non-canonical event by post-training stage, at temperature 1 / top-p 1 and at the recommended settings. 500 rollouts per cell, Wilson 95% intervals. RL-Zero-Math ships no generation config; Think-SFT was run at temperature 1 only.

![Figure 6](figures/fig6_divergence.png)

**Figure 6.** Same text, two id sequences: emitted versus canonical from the span on. Top: how often the next-token argmax differs, by distance after the span. Bottom: logit-lens KL by layer within 16 tokens of the span.
...
| OLMo-3-7B RL-Zero-Math | same, no SFT | RL steps 300, 600, 1400, 2000 (final) [todo: 1000, 1800 pending] | RL directly on the base model; the chat template injects a fixed "solve step by step" instruction |
...
RL-Zero-Math rises across training, from 14.6% at steps 300 and 600 to 28.8% at step 1400 and 58.4% at step 2000 (Figure 2). Most of the rise is one learned span: ` $`+`($` at the third token of the rollout, where the model opens a LaTeX expression right after the template's instruction, in 16, 99 and 210 rollouts at steps 600, 1400 and 2000. Excluding that span the fraction is 14.6%, 12.0%, 11.8% and 26.6%: flat, then a doubling in the last 600 steps. Within the first 1,024 tokens the whole-rollout numbers are 2.8% → 6.0% → 21.6% → 44.3%, so length is not the driver.

![Figure 2](figures/fig2_rlzero_steps.png)

**Figure 2.** RL-Zero-Math over RL steps: rollouts with at least one event, on the whole rollout, within the first 1,024 tokens, and excluding the ` $($` span. [todo: steps 1000 and 1800 when they land.]
...
The obvious alternative explanation is that non-canonical tokens are tail samples that a sane top-p removes. The stored top-10 logprobs say otherwise (Figure 3). In the three cells I ran the logprob analysis on, the first token of the span was the model's argmax 76–80% of the time (Think-DPO 76%, Think RL final 80%, RL-Zero step 2000 79%), against 86–88% of all sampled tokens at rank 1. In Think-SFT and RL-Zero step 300 it is about half. Only 2–11% of spans start beyond the top 10.
...
DPO's bare-space spans have the space at probability near 1 and the token after it at rank >10 in 81% of cases. Excluding those, DPO's argmax-start spans per million argmax samples are 26, against 11 for SFT and 21 for RL final. Sharpness does not explain the ladder either: among the temperature-1 cells, Think-SFT is the least sharp by every measure (top-10 entropy 0.62 nats, 1.2% of samples beyond the top 10) and has the lowest tail-start span rate, while DPO is the sharpest (0.34 nats, 0.5%) and its beyond-top-10 samples are non-canonical 11× more often than SFT's.

The recommended settings lower every OLMo-3 cell by 3.6–8.5× (all p ≤ 7e-4) without changing the ordering: Think-DPO 55% → 15%, Think RL 10% → 2%, Instruct-DPO 21% → 5%, Instruct RL 6% → 1%. In the two recommended-settings cells I measured, the model never sampled beyond its top 10, so what survives is argmax and near-argmax spans; Think-DPO's 15% is still mostly bare-space spans (73 of 108 events), and 4% excluding them.

### 3. Robustness: length, problem set, correctness

Figure 4 restricts the flag to the first 256, 1,024 and 4,096 tokens among rollouts that reached that length. Think-DPO and RL-Zero separate from the other checkpoints within the first 1,024 tokens (Think-DPO 22% against 0.8% for SFT and 1.2% for RL final), so those gaps are not an artefact of length. The window has no power for Instruct-SFT (31 rollouts reach 1,024 tokens) or for Tulu, whose rollouts average about 1,000 tokens.

![Figure 4](figures/fig4_window.png)

**Figure 4.** Flagged fraction within a fixed token budget, among rollouts at least that long. The whole-rollout point is over all 500 rollouts; each window is over the subset that reached it, which for Instruct is a shorter and harder subset.
...
Events could cluster within rollouts because some rollouts are prone or because an event raises the chance of the next one (Figure 5, details in `RESULTS.md`). The large cells have about as many rollouts with two or more events as a Poisson process at the cell's rate would give each rollout's length (Think-DPO 119 against 118 expected), so events are not concentrated in prone rollouts. Consecutive events do sit closer together than random placement in the same rollout in 9 of 12 cells, but restricting to consecutive events with different span text splits the story: in Think RL final, 84% of consecutive events repeat the same text and among the rest the clustering disappears (p = 0.33), likewise in RL-Zero step 2000 (41% repeats, p = 0.58), while in Think-DPO it stays (median gap 1,019 against 1,725 tokens, p < 0.0005). For the on-policy RL checkpoints, "contagion" is a learned tokenization repeating. For Think-DPO, events of different text cluster beyond what its count and length predict, which could be a local state (a stretch of text where the tail is being sampled) or the first event changing the model's state. The next section tests the second.
...
1. **On-policy RL from an SFT model canonicalizes rather than releases.** The sampled ids are overwhelmingly canonical, and reinforcing sampled sequences sharpens the model onto its own canonical habits. RL from the base model has no such prior, and what it reinforces includes its own non-canonical habits (the ` $($` opening is the clearest case: reinforced because it starts correct answers, not because of its tokenization). Test: the Think RL run has 55 intermediate checkpoints; the rate should fall over steps from the DPO starting point.
2. **DPO teaches a tokenizer-specific bare-space habit, and also raises ordinary spans.** Over half of Think-DPO's spans are a standalone space at probability near 1 followed by a tail token, which this tokenizer makes non-canonical unless a digit follows. Something in the preference data or the DPO objective puts mass on that space. Test: look for the pattern in the chosen and rejected completions of the DPO mixture. The 2.3× rise in ordinary spans needs a separate explanation; the sharpness numbers say DPO's tail samples are worse, not more frequent.
...
- **RL-Zero curve** has four points [todo: six when steps 1000 and 1800 land].
