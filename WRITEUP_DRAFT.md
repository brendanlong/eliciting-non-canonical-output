...
Events could cluster within rollouts because some rollouts are prone or because an event raises the chance of the next one (Figure 5, details in `RESULTS.md`). The large cells have about as many rollouts with two or more events as a Poisson process at the cell's rate would give each rollout's length (Think-DPO 119 against 118 expected), so events are not concentrated in prone rollouts. Consecutive events do sit closer together than random placement in the same rollout in 9 of 12 cells, but restricting to consecutive events with different span text splits the story: in Think RL final, 84% of consecutive events repeat the same text and among the rest the clustering disappears (p = 0.33), likewise in RL-Zero step 2000 (41% repeats, p = 0.58), while in Think-DPO it stays (median gap 1,019 against 1,725 tokens, p < 0.0005). The direct test is to take consecutive spans in a rollout and ask whether the second span's first token is more likely under the real context, with the first span as emitted, than under the same context with the first span canonically re-tokenized. For RL-Zero step 2000's same-text pairs it is: 18 of 20 rollouts have a positive median log-probability difference (sign test p = 0.0004, pair-level median +0.85 nats), and the canonical alternative loses about 7 nats. So the repeated habit spans are in-context copying of a tokenization the model already produced, on top of the weight-level habit that produces the first one. For different-text pairs there is nothing: Think-DPO 38 of 85 rollouts positive (p = 1), RL-Zero 23 of 55 (p = 0.34). Think-DPO's clustering of different-text events is not contagion; those events cluster because the passage they sit in keeps the tail live.
...
- **Clustering baselines** control for depth but not for prompt; the direct contagion test rests on 20 rollouts for RL-Zero and on two for Think RL final.
