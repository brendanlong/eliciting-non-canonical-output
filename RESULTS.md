...
**Status (2026-09-04): first measurement, superseded.** These tables come
from the first build of the measurement (commit ada2362), which truncated
the context to the prompt plus the last 4,096 emitted tokens before the
span and required that window to re-encode canonically. Brendan's review
pointed out that a truncated, spliced prefix is out of distribution; the
current code keeps the full real prefix in both sequences and re-tokenizes
only from the span on, and a rerun of every cell with that build is in
progress and will be recorded separately. The numbers below are kept as
the first result, not the final one.
...
re-tokenization of the decoded text (B), identical up to the span, then
the span and up to 512 following tokens. At every byte boundary after the
span that both tokenizations share, starting with the span's own end
(distance 0): the KL divergence KL(A‖B) between the two next-token
distributions, whether the argmax differs, and, at layers 4, 8, …, 28, the
logit-lens KL and the residual-stream cosine distance. Spans with another
event inside the window were skipped (the divergence measured is the
span's own), at most 3 per rollout, 400 per cell. One H100, all 12 DAPO
cells, about 2 hours; records under `divergence/<run>` on the HF dataset
(the parquet files also hold split-word control rows from this build,
which the summaries no longer report).
...
median KL 0.008–0.30 nats in the OLMo-3 cells. The difference decays
fast: by 5–16 tokens after the span the argmax differs at 2.4–4.5% of
boundaries in the OLMo-3 cells, and from 17 tokens on at 0.3–1.7% with
median KL below 0.001. The logit lens near the span: KL and residual
cosine distance are largest at layers 24–28 in six of the eight OLMo-3
cells (Think-SFT and RL-Zero step 2000 peak at layer 4). The Tulu cells
(8–17 spans each) are too small to read beyond the same fast decay;
Tulu-3-SFT shows large mean KL far from the span with medians near zero
(a handful of boundaries inside degenerate rollouts).

*Notes from Claude:* beyond the span the two sequences differ only in
position index (the canonical span usually has a different token count,
so every later token in B sits a position earlier than in A), which is the
likely source of the far-field floor; spans whose alternative tokenization
has the same token count would measure that directly. The final-layer
logit lens of this build double-applied the model's norm, so the
per-layer table stops at layer 28 and the final column is the KL of the
final distribution. Whether these numbers survive the full-prefix rerun
is the open question this section will be replaced by.
